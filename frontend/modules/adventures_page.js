...
function addAdventureToDOM(adventures) {
  ...
  adventures.forEach(({id, name, costPerHead, duration, currency, category, image}) => {
    let card=document.createElement('div');
    card.classList.add('col-6', 'col-lg-3', 'position-relative', 'mb-4');
...
function generateFilterPillsAndUpdateDOM(filters) {
  ...
  filters["category"].filter(function(ele){
    const divele= document.createElement("div");
      divele.className="category-filter";
      const classele = document.getElementById("category-list");

      classele.append(divele);
      return divele;
  })
}
export {
